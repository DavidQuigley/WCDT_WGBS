#Fig1A: overlap_hmr.R
#Fig1B: plot_hmrseg_hmap.R
#Fig1C: plot_box.R
#Fig 2: plot_genes.R
#Fig 3A: expr_methyl_add.R
#Fig 3B: plot_gene.R
#Fig 4A: Uses a file made in expr_methyl_add.R. The plot was made in excel.
#Fig 4B: plot_gene.R
#Fig 4C: plot_tracks.R
#Fig 5A: plot_tracks.R
#Fig 5B: plot_error.R
#Fig 5C: plot_tracks.R
#Fig 5D: plot_error.R
#Fig 6B: window_dss.R
...
fns_chipseq = c(
    ...
    make_path(BASE_DIR, 'secondary_data/chipseq/loc_are_sum_hg38.bed'  ),
...
ensembl2sym = read.table(fn_ensembl2sym, header=T, row.names=1, stringsAsFactors=F, strip.white=T)
...
samples_tet2 = c("DTB-024-PRO","DTB-094-BL","DTB-202-BL","DTB-252-BL") #252 is the one out of CMP
samples_dnmt3b = c("DTB-023-BL","DTB-188-BL")
...
# Gene lists
#------------------------------------------------------------------------------------------------------
cosmic = read.table( fn_genelists, sep='\t', header=T, stringsAsFactors=F, strip.white=T)
...
#------------------------------------------------------------------------------------------------------------------
cellsummary = read.delim(fn_wgs_sample_summary,row.names=1,header=T,sep='\t',stringsAsFactors=F,strip.white=T)
...
#------------------------------------------------------------------------------------------------------------------
data_manta_slim = read.table(fn_manta_slim, sep='\t', 
                              header=T, row.names=1, stringsAsFactors=F, strip.white=T)
data_manta_slim$sample_id = gsub('_manta.vcf', '', data_manta_slim$sample_id, fixed=T)

rows_manta_pass = data_manta_slim$normal_reads==0 &
    (data_manta_slim$tumor_reads_pair+data_manta_slim$tumor_reads_span)>=10
rows_manta_pass[is.na(rows_manta_pass)] = F

manta_counts = data.frame()
manta_samples = unique(data_manta_slim$sample_id)
for(manta_sample in manta_samples) {
    rowsmantasample = data_manta_slim$sample_id==manta_sample
    manta_counts[manta_sample,'SV'] = sum(rowsmantasample)
    mantatab = table(data_manta_slim[rowsmantasample,'svtype'])
    for(j in 1:length(mantatab)) {
        manta_counts[manta_sample,names(mantatab)[j]] = mantatab[j]
    }
}
colnames(manta_counts) = gsub('Manta','',colnames(manta_counts),fixed=T)
...
for(i in 1:length(filenames_cc)) {
	...
	data_i = read.table(filename_bed, header=F, stringsAsFactors=F, sep='\t')
	sample_i = gsub('_copycat.bed','',filenames_cc[i],fixed=T)
...
#------------------------------------------------------------------------------------------------------------------
annovar = read.delim(fn_annovar, header=T, sep='\t', stringsAsFactors=F, strip.white=T)
...
for(i in 1:length(namelist_chipseq)) {
    ...
    track = read.delim(filename, header=F, sep='\t', stringsAsFactors=F, strip.white=T)
...
promoterpos = cbind.data.frame(ensembl2sym$chr,ensembl2sym$start-PROMOTER_RANGE,ensembl2sym$start+PROMOTER_RANGE,
                                ensembl2sym$strand,ensembl2sym$name)[rowspos,]
promoterneg = cbind.data.frame(ensembl2sym$chr,ensembl2sym$end-PROMOTER_RANGE,ensembl2sym$end+PROMOTER_RANGE,
...
tracks[['promoter']] = makeGRangesFromDataFrame(promoterdf,keep.extra.columns=T)

genebody = cbind.data.frame(ensembl2sym$chr,ensembl2sym$start,ensembl2sym$end,
                             ensembl2sym$strand,ensembl2sym$name)
colnames(genebody) = c('chr','start','end','strand','gene')
tracks[['genebody']] = makeGRangesFromDataFrame(genebody,keep.extra.columns=T)

upstreampos = cbind.data.frame(ensembl2sym$chr,ensembl2sym$start-RANGE_UP_DOWN,ensembl2sym$start,
                                ensembl2sym$strand,ensembl2sym$name)[rowspos,]
upstreamneg = cbind.data.frame(ensembl2sym$chr,ensembl2sym$end,ensembl2sym$end+RANGE_UP_DOWN,
                                ensembl2sym$strand,ensembl2sym$name)[rowsneg,]
...
tracks[['upstream']] = makeGRangesFromDataFrame(upstreamdf,keep.extra.columns=T)
...
tracks[['downstream']] = makeGRangesFromDataFrame(downstreamdf,keep.extra.columns=T)

### CpG islands/shores/shelves

cpg = read.delim(fn_cpg, header=T, sep='\t', stringsAsFactors=F, strip.white=T)
...
tracks[['HMRseg']] = makeGRangesFromDataFrame(hmrseg,keep.extra.columns=T)
...
for(i in 1:length(final_hmr_list)) {
    ...
    hmr[[sample]] = makeGRangesFromDataFrame(as.data.frame(final_hmr_list[[i]]),keep.extra.columns=T)
...
array27kdf = read.delim(fn_array_27k,header=T,sep='\t')
array27k = makeGRangesFromDataFrame(array27kdf)
array450kdf = read.delim(fn_array_450k,header=T,sep='\t')
array450k = makeGRangesFromDataFrame(array450kdf)
epicdf = read.delim(fn_array_epic,header=T,sep='\t')
