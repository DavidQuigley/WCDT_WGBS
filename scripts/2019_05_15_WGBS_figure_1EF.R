...
plist = ggplot(shoreshelf,aes_string(x='groups', y="count") )+
    xlab('') + ylab( '' ) +
...
ggsave(fn_figure1e,width=2, height=3,useDingbats=FALSE)


plist = ggplot(df,aes_string(x='groups', y="n_outside_shelf") )+
    xlab('') + ylab( '' ) +
    geom_boxplot( outlier.shape=NA) +
    geom_jitter( size=1, position=position_jitter(height=0,width=.2), color=df$colors) +
    theme_classic() +
    scale_y_continuous( limits=c(0,70), breaks=seq(0,80,20), expand = c(0, 0))

#ggarrange(plotlist=plist,ncol=length(plist),nrow=1)
ggsave(fn_figure1f,width=2, height=3,useDingbats=FALSE)
