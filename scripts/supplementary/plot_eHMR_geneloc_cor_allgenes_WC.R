...
savedest <- '/home/will/Desktop/wgbs/results/methylval.plots/gene_norm'
curdate <- '2019-05-10'

##################
# Load data
##################
hmr.tab <- read.delim(file='/home/will/Desktop/wgbs/data/hmrseg_2019mar/hmrseg_gene.txt', sep='\t', header=T, stringsAsFactors = F)
ensembl2sym <- read.delim(file='/home/will/Desktop/wgbs/dependencies/ensembl2sym.txt', sep='\t', header=T, stringsAsFactors=F)
expressed.genes <- read.delim(file='/home/will/Desktop/wgbs/dependencies/expressed.txt', sep='\t', header=F, stringsAsFactors=F)
...
fname <- sprintf('%s/%s/neg-eHMR-cor-allgenes.pdf', savedest, curdate)
ggsave(filename=fname, plot=curplot, width=8, height=5, device='pdf')
...
fname <- sprintf('%s/%s/neg-eHMR-cor-density-allgenes.pdf', savedest, curdate)
ggsave(filename=fname, plot=curplot, width=8, height=5, device='pdf')
...
fname <- sprintf('%s/%s/pos-eHMR-cor-allgenes.pdf', savedest, curdate)
ggsave(filename=fname, plot=curplot, width=8, height=5, device='pdf')
...
fname <- sprintf('%s/%s/pos-eHMR-cor-density-allgenes.pdf', savedest, curdate)
ggsave(filename=fname, plot=curplot, width=8, height=5, device='pdf')
