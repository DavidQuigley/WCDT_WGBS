...
outputfile <- 'C:/Users/Admin/Desktop/valley_5kb_bar.pdf'
print(outputfile)
...
output <- output[rownames(output) %in% samples_wgbs,]
...
df3 <- cbind.data.frame(sample=output$Sample,count=output$width/hg38size*100)
...
ggsave(outputfile,width=15,height=12)
dev.off()
