...
#same expanses as the other plots

start = gene_start+3100000
end = gene_end+2930000
samples2use = sample_ids_wgbs[allele_effect('ERG')$alleles[sample_ids_wgbs,'activating_sv']]
#use this line to plot the ERG-fusion positive samples
samples2use = intersect(samples2use,sample_ids_wgbs)
#use this line to plot the ERG-fusion negative samples
...
arexpr = as.numeric(tpm['ENSG00000169083.16',samples2use])
armut = allele_effect('AR')$alleles[samples2use,'activating_missense']
hmrgene = hmrsegs[countOverlaps(hmrsegs,gr)>0]


covs$add1 <- armut
covs$add2 <- arexpr
add2name <- 'AR expr+mut'
...
for(i in 1:length(hmrgene)) {
    ...
    methylmean = meanmethyl(ensemblid,dir_gene_output_mcrpc,
                            hmr_start,hmr_end)[samples2use]
    covs[,i+2+as.numeric(genename=='ERG')] = methylmean
}

lm1 = lm(expr~add1+add2,covs)
print(summary(lm1))
lm2 = lm(expr~.,covs[,-1])
...
ggsave(fn_figure5b,p,width=7,height=7,useDingbats=F)
